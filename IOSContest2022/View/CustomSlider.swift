...
struct SliderBack: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let rad1 = rect.midY-rect.minY
        let rad2 = rad1 / 12
        
        path.move(to: CGPoint(x: rect.minX + rad2, y: rect.midY - rad2))
        path.addLine(to: CGPoint(x: rect.maxX - rad1, y: rect.minY))
        path.addArc(center: CGPoint(x: rect.maxX - rad1, y: rect.midY), radius: rad1, startAngle: Angle(degrees: 270), endAngle: Angle(degrees: 90), clockwise: false)
        path.addLine(to: CGPoint(x: rect.maxX - rad1, y: rect.maxY))
        path.addLine(to: CGPoint(x: rect.minX + rad2, y: rect.midY + rad2))
        path.addArc(center: CGPoint(x: rect.minX + rad2, y: rect.midY), radius: rad2, startAngle: Angle(degrees: 90), endAngle: Angle(degrees: 270), clockwise: false)
        path.addLine(to: CGPoint(x: rect.minX + rad2, y: rect.midY - rad2))
...
struct CustomSlider_Previews: PreviewProvider {
    static var previews: some View {
        CustomSlider(value: .constant(8.0),
                     in: 2...24,
